...
Every command below is run from the repository root on NERSC (`cd /global/cfs/cdirs/atlas/<user>/cCKF`) unless it says otherwise. `python -m pytest tests -q` runs the whole Python suite (413 tests, about 15 s, no data needed).

### 1. Classical CKF optimisation

I provide notes in the `NEXT_STEPS.MD` section but I think it is worthwhile to conduct a more comphrehensive optimization of the CKF using Chance's paper `~/priorOptimizationPaper.pdf` and EHVI (expected hypervolume improvement over a broader set of optimization parameters). The logic being seeding is so important to the dataset generation and later evaluation of the gate and value function. 

- **Code:** `modal_apps/modal_acts.py`, functions `run_seeding_optimizer`, `run_ckf_optimizer`, `run_joint_motpe_optimizer`, `validate_joint_motpe_eval` (Optuna; the TPE (Tree Parzen Estimator) sampler stands in for MOTPE (Multi-Objective-TPE) in Optuna 4). The resulting operating points are `configs/tight_t79.yaml`, `configs/medium_t70.yaml`, and the `configs/_motpe_`* harness configs. Trials, evaluations and per-event results are in `experiments/joint_motpe/`; the full record is Part I of `experiments/LOG.md` 
...
These are the insertions which serve to pull out the cluster shapes, incidence angle, and charge deposited from ACTS to construct the feature vector for the gate function. This also contains the use of rsyn to only re-compile and link the files which have changed as opposed to rebuilding all of the libraries in ACTS and relinking them. 
...
- Tests: two standalone Cparity tests, no ACTS needed: `clang -std=c++17 -O2 -Iacts_patches/cckf tests/test_mlp_inference.cpp -o /tmp/t && /tmp/t`, likewise `tests/test_chi2_logodds.cpp`. Their fixtures are regenerated by `tests/test_export_weights.pyandtests/test_features.py. scripts/instrumentation/check_trackstate_branches.py` verifies the patched branches in a real ROOT file.
...
This is the process of running the CKF with the insertions and getting a full ROOT file with all of measurements selected by the CKF. These measurements have to later be "re-expanded" to generate the full training data, which requires writing specific CSVs and allows you to turn off terminal cuts. 
...
The process of expansion is simply taking each predicted measurement, taking a 10-sigma window around that measurement, and collecting all of the same measurement features for all of the other hits the CKF could have chosen at that surface. The output we want is a set of (x, y) data and label pairs where the features contain all statistics from the measurement and the branch history, and the label y is 1 if the measurement was from the branches majority particle. 

- **Code:** `expansion.py`, entry `run_expansion(trackstates_root, csv_dir, event_id, output_path, digi_config_path=...)`, which joins ROOT states to every in-window candidate and computes labels, cluster features, branch history, and the seed-majority particle. Then `scripts/patch_is_selected.py --parquet P --root R --event-id E --out P2` marks the CKF-accepted candidate per state.
...
The gate function aims to answer the question of, given a branch and candidate hit, what is the probabilty that hit is part of the branch, concretely that a given cluster was formed from that particle and possibly other particles. 
...
The value function aims to answer the question, given a branch, how likely is that branch to complete to a double majority matched track. Constructing the labels for the value function involves re-propagating the CKF forward from surface's where it choose an incorrect hit. 
...
There are a few sweeps you can run. One is finding a pareto front of the gate and value function, where we are optimizing against efficiency, latency, and fake rate. The variables are the thresholds $\tau_g$ and $\tau_v$ below which a hit is rejected by the gate function or a branch is pruned by the value function. 
...
│   │                         and the truth-greedy rollout selector used for tier-2 (re-propagated) targets
...
│                             [P7] classify branch states, emit rollout worklists, compose tier-2 (re-propagated)
│                             targets; file names carry the historical "tier3" code name
│
...
│   │                         [P7] re-propagation (tier-2) rollout generation per window, stitching with the truth-suffix gate
