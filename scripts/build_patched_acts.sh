...
# Headers, CMakeLists surgery and the pybind registration are shared with
# scripts/build_cckf_nersc.sh via one script. The pybind block enumerates
# every Config field by name; a second copy would drift silently, since a
# missing field is ignored at runtime rather than erroring.
ACTS_SOURCE="$ACTS_SOURCE" CCKF_REPO="/app" \
    bash /app/scripts/apply_cckf_integration.sh
