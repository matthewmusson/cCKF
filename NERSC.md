...
marked otherwise. `<user>` is your NERSC username. Your otp (one time password) you have to set up with a third party authentication app like apple passwords. 
...
[https://docs.nersc.gov/connect/mfa/#sshproxy](https://docs.nersc.gov/connect/mfa/#sshproxy) .
...
Units in Iris ([https://iris.nersc.gov](https://iris.nersc.gov)) are **node-hours**. Matthew's
...
90-minute audit, the 3-hour regenerations, the re-propagation array), `interactive`
...
Naming, locked 2026-09-14: **tier 2** is the value target obtained by
re-propagating the CKF from the last correct state (walker + truth rollouts
+ stitching); **tier 3** is the first target we used, computed from the
logged branch alone. The code still uses its historical names
(`tier3_walker.py`, `stitch_tier3.py`, `vstar_t2`); `CLAUDE.md` has the
mapping. Directory names below follow the locked naming.
...
tier2/{worklists,hits}/           re-propagation (tier-2) worklists and rollout hits per event;
                                  `tier3` is a symlink to it because the job files still use the code name
...
cckf_handoff/                     the 37 GB handoff bundle, group-readable:
    ...
    tier2_event4/{worklists,hits} one event's re-propagation worklist and rollout hits
    tier2_targets/                stitched re-propagated (tier-2) value targets
