...
Schema of every file produced by Stage 1 (digitisation + CKF with all writers enabled), and how the expansion pipeline joins them into the per-event Parquet.  
  
Revised 2026-09-10. 

**Worked example on NERSC (group-readable):**  `/global/cfs/cdirs/atlas/mussonm/cckf_handoff/examples/envelope_event4/` holds
...
The raw file has **five** particle-id columns (`particle_id_pv, _sv, _part, _gen, _subpart`) plus `geometry_id, tx, ty, tz, tt, tpx, tpy, tpz, te, deltapx/py/pz, deltae, index`. `expansion.load_simhits` returns
...
`geometry_id, geometry_type, volume_id, layer_id, module_id, cx, cy, cz, rot_*, pitch_u, pitch_v, thickness`. Endcap ids here carry `extra = 0`, unlike
